...
def currency_page():
    ...
    if request.method == "POST":
        output= currency.convert(request.form['input_num'], currencies[request.form['from']], currencies[request.form['to']])
        output = round(output, 2)
        return render_template("currency.html", currencies=currencies, input=request.form['input_num'], output=output)
