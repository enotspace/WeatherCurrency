...
def post_detail(id):
    ...
    if not request.cookies.get(f"article {id, article.title, article.date}"): 
        article.views+=1
        db.session.commit()
        res = make_response(render_template("post_detail.html", article=article))
        res.set_cookie(f"article {id, article.title, article.date}", "yes")
        return res
...
def post_delete(id):
    if request.method == "POST":
        if request.form['password'] == '1234':
            ...
            db.session.commit()
            res = make_response(redirect('/posts'))
            res.set_cookie(f"article {id, article.title, article.date}", "", 0)
            return res
